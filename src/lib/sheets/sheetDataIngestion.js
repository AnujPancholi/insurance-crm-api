...
const readRowWise = (deps) => {
  const { Exceljs, parentPort, logger } = deps;
      ...
      worksheet.getRow(rowNum).eachCell((cell, colNo) => {
        ...
        if (insertConfig) {
          insertConfig.forEach((cfg) => {
            ...
            docObjs[cfg.collection][cfg.keyname] = DATATYPE_UTILS[
              cfg.type
            ].convert(cell.value);
          });
        }
  ...
  return async (rowData) => {
    ...
    try {
      for (const collectionName of DATA_INGESTION_SHEET_COLLECTIONS) {
        ...
        logger.info(
          `Upsert triggered: Rownum:${rowData.rowNum},collection:${collectionName}`
        );
        upsertResults[collectionName] = await db
        ...
        logger.info(
          `Upsert complete: Rownum:${rowData.rowNum},collection:${collectionName}`
        );
      }
    } catch (e) {
      logger.error(
        `Error in ingesting rowdata: RowNum:${rowData.rowNum} : ${e.message}`
      );
      return {
